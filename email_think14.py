...
def fetch_emails_in_time_range_uid(mail, start_hour=8, end_hour=14):
    """
    使用 UID 模式检索收件箱中所有邮件，
...
def main():
    ...
    try:
        ...
        print("检查今日邮件...")
        emails = fetch_emails_in_time_range_uid(mail, start_hour=8, end_hour=14)
        ...
        # 构造回复邮件的主题
        reply_subject = f"早上随想总结 - {datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d')}"
