...
def fetch_all_inbox_emails_uid(mail):
    """
    使用 UID 模式检索收件箱中所有邮件，
...
def main():
    ...
    try:
        ...
        print("检查收件箱中的所有邮件...")
        emails = fetch_all_inbox_emails_uid(mail)  # 使用新函数
        ...
        # 构造回复邮件的主题
        reply_subject = f"全天随想总结 - {datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d')}"
