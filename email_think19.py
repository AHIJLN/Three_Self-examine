...
def fetch_emails_in_time_range_uid(mail, start_hour=14, end_hour=19):
    """
    使用 UID 模式检索收件箱中所有邮件，
...
def main():
    ...
    try:
        ...
        print("检查今日邮件...")
        emails = fetch_emails_in_time_range_uid(mail, start_hour=13, end_hour=18)
        ...
        # 构造回复邮件的主题
        reply_subject = f"下午随想总结 - {datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d')}"
